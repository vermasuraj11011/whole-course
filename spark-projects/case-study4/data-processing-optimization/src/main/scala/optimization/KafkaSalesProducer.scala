...
//import akka.kafka.scaladsl.Producer
//import akka.stream.scaladsl.Source
//import optimization.SalesRecord
//import org.apache.kafka.clients.producer.ProducerRecord
//import org.apache.kafka.common.serialization.{ByteArraySerializer, StringSerializer}
//import scalapb.GeneratedMessage
//
...
//  ): SalesRecord.SalesRecord =
//    SalesRecord
//      .SalesRecord(store = store, department = dept, date = date, weeklySales = weeklySales, isHoliday = isHoliday)
//
//  private def buildRecord(): ProducerRecord[String, Array[Byte]] = {
//    val store       = s"${scala.util.Random.nextInt(10) + 1}"
//    val dept        = s"${scala.util.Random.nextInt(20) + 1}"
//    val date        = java.time.LocalDate.now().toString
//    val weeklySales = 1000 + scala.util.Random.nextFloat() * 1000
//    val isHoliday   = scala.util.Random.nextBoolean()
//
